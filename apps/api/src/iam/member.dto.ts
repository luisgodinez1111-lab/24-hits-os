import { z } from "zod";

export const inviteMemberSchema = z.object({
  email: z.string().email().max(254),
  // El nombre es opcional: un string vacío ("") se trata como ausente (el front lo
  // manda vacío cuando no se llena) para no rechazar la invitación con "Datos inválidos".
  name: z.preprocess(
    (v) => (typeof v === "string" && v.trim() === "" ? undefined : v),
    z.string().min(1).max(120).optional()
  ),
  roleIds: z.array(z.string().uuid()).min(1, "Asigna al menos un rol"),
});
export type InviteMemberInput = z.infer<typeof inviteMemberSchema>;

export const updateMemberRolesSchema = z.object({
  roleIds: z.array(z.string().uuid()).min(1, "Asigna al menos un rol"),
});
export type UpdateMemberRolesInput = z.infer<typeof updateMemberRolesSchema>;

export const setMemberStatusSchema = z.object({
  status: z.enum(["ACTIVE", "SUSPENDED"]),
});
export type SetMemberStatusInput = z.infer<typeof setMemberStatusSchema>;

// Almacén fijo del usuario (null = sin almacén asignado).
export const setMemberWarehouseSchema = z.object({
  defaultWarehouseId: z.string().uuid().nullable(),
});
export type SetMemberWarehouseInput = z.infer<typeof setMemberWarehouseSchema>;
